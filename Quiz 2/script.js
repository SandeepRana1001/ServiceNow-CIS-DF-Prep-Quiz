(function () {
  ...
  QUIZ_DATA.forEach(q => {
    state[q.id] = {
      ...
      selected: q.type === "multiple" ? new Set()
               ...
               : q.type === "matchmulti" ? (() => {
                   const obj = {};
                   q.pairs.forEach((p, i) => { obj[i] = new Array(p.right.length).fill(""); });
                   return obj;
                 })()
               : q.type === "grid" ? new Set()
               : null,
  ...
  function isAnswered(q) {
    ...
    if (q.type === "matchmulti") return Object.values(st.selected).some(arr => arr.some(v => v));
  ...
  function isFullyAnswered(q) {
    ...
    if (q.type === "matchmulti") return q.pairs.every((p, i) => st.selected[i].every(v => v));
    if (q.type === "grid") return st.selected.size > 0;
    return false;
  }
  ...
  function typeLabel(t) {
    return { single: "Single Choice", multiple: "Multiple Choice", match: "Matching", matchmulti: "Matching (multi)", grid: "Multi-Select Grid" }[t];
  }
  ...
  function renderMatchMulti(q, st) {
    ...
    const rowsHtml = q.pairs.map((p, i) => {
      ...
      if (st.submitted) {
        ...
        ok = selSet.size === corSet.size && [...selSet].every(v => corSet.has(v));
        rowCls += ok ? " reveal-correct" : " reveal-incorrect";
        feedback = ok ? `<span class="match-feedback ok">✓</span>` : `<span class="match-feedback bad">✕</span>`;
      }
      const dropsHtml = vals.map((val, slotIdx) => {
        const dropCls = "match-drop matchmulti-drop" + (val ? " filled" : "") + (st.submitted ? " locked" : "");
        const dropContent = val
      ...
      const hint = (st.submitted && !ok) ? `<div class="correct-hint">Correct: ${p.right.join(", ")}</div>` : "";
      return `<div class="${rowCls}">
        <div class="match-left">${p.left}${hint}</div>
  ...
  function attachHandlers(q, st) {
    ...
    if (q.type === "single") {
      questionCard.querySelectorAll('input[name="single-opt"]').forEach(inp => {
      ...
      questionCard.querySelectorAll('.option input[type="checkbox"]').forEach(inp => {
        inp.addEventListener("change", () => {
      ...
      questionCard.querySelectorAll(".drop-clear[data-clear-idx]").forEach(btn => {
      ...
      questionCard.querySelectorAll(".drop-clear[data-clear-row]").forEach(btn => {
        btn.addEventListener("click", (e) => {
          e.stopPropagation();
          clearSlot(parseInt(btn.dataset.clearRow, 10), parseInt(btn.dataset.clearSlot, 10));
        });
      });
    } else if (q.type === "grid") {
      questionCard.querySelectorAll('input[type="checkbox"][data-key]').forEach(cb => {
  ...
  function gradeQuestion(q, st) {
    ...
    if (q.type === "matchmulti") {
      return q.pairs.every((p, i) => {
        ...
        const corSet = new Set(p.right);
        return selSet.size === corSet.size && [...selSet].every(v => corSet.has(v));
      });
    }
    if (q.type === "grid") {
